...
bool compareFunction(int ***ptrPrincipal, int *ptrCond, int Pos1, int Pos2, int dimensionMatrixToRotate, int AmountConditions, int *ptrStates, int *ptrorden,bool bajar_dimension,int InitialSizeIteration1){
    ...
    int amountof1 = 0, state = 0, contRotations = 0, WichCond = 0, InicialSize,cont1, cont2, pAux1, pAux2, out,ini_pos1,ini_pos2;
    ...
    while(WichCond<AmountConditions){
        ...
        case 0:
            cambiar_dimension_matriz(ptrPrincipal,ptrorden[WichCond],cont2,dimensionMatrixToRotate);
            dimensionMatrixToRotate += ptrorden[WichCond] - InicialSize;
            while(true){
                if(ptrPrincipal[cont1][pAux1][ pAux2] == ptrPrincipal[cont2][ pAux1][pAux2]){
                    // poner cada estado y dimension en arreglos
            ...
            while(true){
                ...
                if (ptrPrincipal[cont1][pAux1][ pAux2] > ptrPrincipal[cont2][Pos1][Pos2]){
                    ...
                    if(contRotations<3){
                        ...
                        Pos1=ini_pos1;
                        Pos2=ini_pos2;
                        Repetition = compareFunction(ptrPrincipal,ptrCond,Pos1,Pos2,InicialSize+(amountof1*2),AmountConditions,ptrStates,ptrorden,true,InitialSizeIteration1);
